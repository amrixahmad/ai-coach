...
- **Frontend API Config & Tab State**: Created `ApiConfig` in [api_config.dart](file:///c:/Users/user/Documents/ai-coach/pickleball_app/lib/data/services/api_config.dart) defaulting to Railway Production API, with `--dart-define=API_URL=...` override for local dev testing. Added navigation tab persistence in `main_shell.dart` so page refreshes maintain active tab position (History/Profile).
...
- **Cloudflare R2 Object Storage & Cleanup**: Integrated `storage.py` using `boto3` in `process_video`. Automatically uploads video clips to Cloudflare R2 when `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, and `R2_ACCOUNT_ID` / `R2_PUBLIC_DOMAIN` environment variables are set, with seamless fallback to static file storage when unconfigured. Automated object deletion (`delete_file_from_r2`) purges videos from R2 when a session is deleted.
